...
const (
	...
	ErrApplicationConversionCode = "2172"
)
...
func ErrApplicationConversion(err error) error {
	return errors.New(ErrApplicationConversionCode, errors.Alert, []string{"Error converting application to a pattern file"}, []string{err.Error()}, []string{"Invalid syntax in the passed format"}, []string{"Make sure the syntax is proper. For eg- If the format is of type kubernetes, make sure it is a valid kubernetes yaml"})
}
