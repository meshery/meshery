...
func (h *Handler) MeshAdapterConfigHandler(w http.ResponseWriter, req *http.Request) {
	...
	case http.MethodPost:
		...
		if strings.TrimSpace(meshLocationURL) == "" {
			err := errors.New("meshLocationURL cannot be empty to add an adapter")//strings should not be capitalized or end with punctuation or a newline
			logrus.Error(err)
...
func (h *Handler) deleteAdapter(meshAdapters []*models.Adapter, w http.ResponseWriter, req *http.Request) ([]*models.Adapter, error) {
	...
	if aID < 0 {
		err := errors.New("Unable to find a valid adapter for the given adapter URL") //strings should not be capitalized or end with punctuation or a newline
		logrus.Error(err)
...
func (h *Handler) MeshOpsHandler(w http.ResponseWriter, req *http.Request) {
	...
	operationID, err := uuid.NewV4()  //updated operationId to operationID
	if err != nil {
