 ...
 *  handleSubmit: (data: any, _id: string, name: string, type: string) => void,
...
function FiltersGrid({
  ...
  const handlePublishModal = (filter: any) => {
    if (canPublishFilter) {
      setPublishModal({
        open: true,
        filter: filter,
  ...
  const getYamlConfig = (filter_resource: string) => {
    if (filter_resource) {
