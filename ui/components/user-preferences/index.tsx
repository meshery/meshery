...
const RemoteProviderInfoTab: React.FC<RemoteProviderInfoTabProps> = ({
  providerInfo,
  userData,
}) => {
  return (
        ...
        <RootContainer>
          ...
          <ErrorBoundary>
            <Grid2 container spacing={2} size="grow">
              {providerInfo &&
                Object.entries(providerInfo).map(([providerName, provider], index) => {
                  const providerText =
          ...
          <Grid2 container size="grow" style={{ display: 'flex', justifyContent: 'space-between' }}>
            <GridCapabilityHeader
              size={{ xs: 6 }}
