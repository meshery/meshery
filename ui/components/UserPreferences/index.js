...
export const StyledTabs = styled(Tabs)(({ theme }) => ({
  ...
  '& .MuiTab-root': {
    ...
    color: theme.palette.mode === 'dark' ? theme.palette.text.default : theme.palette.text.primary,
    '&.Mui-selected': {
      backgroundColor: 'transparent',
      color:
        theme.palette.mode === 'dark'
...
const UserPreference = (props) => {
  ...
  const handleCatalogContentToggle = () => {
    const newCatalogContent = !catalogContent;
    console.log('handleCatalogContentToggle - New state:', newCatalogContent);
    setCatalogContent(newCatalogContent);
    props.toggleCatalogContent({ catalogVisibility: newCatalogContent });
    handleCatalogPreference(newCatalogContent);
  };

  const handleCatalogPreference = (newCatalogContent) => {
    console.log('handleCatalogPreference - Updating preference to:', newCatalogContent);
    let body = { ...extensionPreferences, catalogContent: newCatalogContent };
    updateUserPref({ usersExtensionPreferences: body })
      .unwrap()
      .then(() => {
        console.log('API call successful. New state:', newCatalogContent);
        notify({
          message: `Catalog Content was ${newCatalogContent ? 'enabled' : 'disabled'}`,
          event_type: EVENT_TYPES.SUCCESS,
        });
        // Add a delayed log to check the state after the API call
        setTimeout(() => {
          console.log('State 1 second after successful API call:', catalogContent);
        }, 1000);
      })
      .catch((error) => {
        console.error('API call failed:', error);
        handleError('There was an error sending your preference');
        console.log('Reverting state to:', !newCatalogContent);
        setCatalogContent(!newCatalogContent);
      });
  };
