...
import { withStyles, TextField, Grid, Button, IconButton } from '@material-ui/core/';
...
const MetricsConfig = ({
  ...
  const handleConfigurationSubmit = () => {
    if (componentName === 'Grafana')
      handleGrafanaConfigure(notify, url, apiKey, updateProgress, updateGrafanaConfig);
    if (componentName === 'Prometheus')
      handlePrometheusConfigure(notify, url, updateProgress, updatePrometheusConfig);
  };
  const getOptions = () => {
    if (componentName === 'Grafana')
        ...
        <Grid item xs={12} style={{ height: '13rem' }}>
          ...
          <Grid item xs={12}>
            ...
            <Grid
              item
              xs={12}
              style={{ marginTop: '0.8rem', marginBottom: '0.8rem', textAlign: 'center' }}
            >
              <Button
                type="submit"
                variant="contained"
