...
const styles = () => ({
  ...
  cardChecked: { height: '15rem', marginBottom: '1rem' },
  ...
  return (
    <Container className={classes.cardContainer} key={serviceInfo.name}>
      <Card className={`${classes.card} `} variant="outlined">
        <CardContent
          className={showConfigComponent() ? classes.cardContentChecked : classes.cardContent}
        >
          <div className={isConnected ? classes.contentTopConnected : classes.contentTop}>
            <div className={classes.iconContainer}>
              <ServiceIcon isActive={isConnected} />
              <Typography
                className={isConnected ? classes.cardIconTextConnected : classes.cardIconText}
                color="primary"
              >
                {serviceInfo.name}
              </Typography>
          ...
          {showConfigComponent() && (
            <div style={{ paddingLeft: '0.6rem', paddingRight: '0.6rem' }}>{ConfigComponent}</div>
          )}
        </CardContent>
      </Card>
    </Container>
