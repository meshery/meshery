...
//   updateCapabilities : bindActionCreators(updateCapabilities, dispatch),npm 
// });
...
function Navigator(props) {
  // Initialize component state using useState hooks
  ...
  // Component lifecycle methods are replaced with useEffect
  useEffect(() => {
    ...
    dataFetch(
      ...
      (result) => {
        if (typeof result !== "undefined") {
          setState({ ...state, versionDetail: result });
        } else {
          setState({
            ...state,
            versionDetail: {
  ...
  // Define other component methods as regular functions
  function isServiceMeshActive() {
    return state.meshAdapters.length > 0;
  }

  function renderNavigatorExtensions(children, depth) {
  ...
  function extensionPointContent(icon, href, name, drawerCollapsed) {
    const { classes } = props;
  ...
  function updatenavigatorComponentsMenus() {
    const self = this;
  ...
  function fetchChildren(category) {
    ...
    meshAdapters.forEach((adapter) => {
      ...
      children.push({
        id: adapter.adapter_location,
        icon: adapter.adapter_ui_config?.icon ?? "",
        href: adapter.adapter_ui_config?.url ?? "",
        title: adapter.adapter_ui_config?.name ?? "",
      });
    });
  ...
  function handleServiceMeshClick() {
    setState((prevState) => ({
      ...prevState,
      openItems: ["LIFECYCLE"],
      path: "LIFECYCLE"
    }));
    props.history.push("/extensions");
  }
  ...
  const { classes, isDrawerCollapsed, history } = props;
  const { navigatorComponents, openItems, hoveredId } = state;
  const { path } = state;
  return (
    <NoSsr>
      <Drawer
        ...
        className={classNames(classes.drawer, {
          ...
          [classes.drawerExpand]: !isDrawerCollapsed
        })}
      >
        <List>
          {navigatorComponents.map(({
            id, icon, href, title, children, show: showc, link
          }) => {
            if (typeof showc !== "undefined" && !showc) {
              return "";
            }
              ...
              <div key={id}>
                <ListItem
                  className={classNames(
                    ...
                    path === id && classes.itemActiveItem,
                    isDrawerCollapsed && classes.noPadding
                  )}
                  onClick={() => {
                    if (hasChildren) {
                      const openItemsCopy = openItems.slice();
                      if (openItemsCopy.includes(id)) {
                        const index = openItemsCopy.indexOf(id);
                        if (index > -1) {
                          openItemsCopy.splice(index, 1);
                        }
                      } else {
                        openItemsCopy.push(id);
                      }
                      setState((prevState) => ({
                        ...prevState,
                  ...
                  {hasChildren ? (
                    <IconButton
                      className={classNames(classes.expandIcon, {
                        [classes.hideExpandIcon]: !isDrawerCollapsed
                      })}
                    >
                      {openItems.includes(id) || (hoveredId === id && isDrawerCollapsed) ? (
                        <FontAwesomeIcon
                          icon={faAngleLeft}
                        />
                      ) : (
                ...
                {renderNavigatorExtensions(children, 1)}
                {hasChildren ? (
                  <Divider className={classes.divider} />
                ) : null}
              </div>
          ...
          <ListItem
            ...
            className={classNames(
              classes.item,
              classes.itemCategory,
              path === ABOUT_NAME && classes.itemActiveItem
            )}
            onClick={() => updatePath(ABOUT_NAME)}
          >
            {extensionPointContent(
              `${globalServerConfig?.ui?.extensionUiUri || '/static/img/meshery/icon.png'}`,
              "/about",
              ABOUT_NAME,
              isDrawerCollapsed
            )}
        ...
        <Collapse
          in={isServiceMeshActive() && state.navigator && state.navigator[0].id === "ABOUT"}
        >
          <div className={classes.serviceMeshAdaptersContainer}>
            <div
              className={classNames(classes.serviceMeshAdapters, {
                [classes.noPadding]: isDrawerCollapsed
              })}
            >
              <ListItem
        ...
        <ListItem
          ...
          className={classNames(
            classes.item,
            classes.itemCategory,
            path === "Documentation" && classes.itemActiveItem
          )}
          onClick={() => updatePath("Documentation")}
        >
          {extensionPointContent(
            <HelpIcon />,
            "/docs",
        ...
        <ListItem
          ...
          className={classNames(
            classes.item,
            classes.itemCategory,
            path === "Community" && classes.itemActiveItem
          )}
          onClick={() => updatePath("Community")}
        >
          {extensionPointContent(
            <HelpIcon />,
            "/community",
        ...
        <ListItem
          ...
          className={classNames(
            classes.item,
            classes.itemCategory,
            path === "About Meshery" && classes.itemActiveItem
          )}
          onClick={() => updatePath("About Meshery")}
        >
          {extensionPointContent(
            <HelpIcon />,
            "/about",
          ...
          >
            {state.showHelperButton && (
              ...
              >
                ...
                <IconButton
                  onClick={() => {
                    const url = `${globalServerConfig?.ui?.extensionUiUri || '/static/img/meshery/icon.png'}`;
...
export default withStyles(useStyles, { withTheme: true })(MesheryNavigator);