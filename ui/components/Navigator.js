...
const styles = (theme) => ({
  ...
  item : {
    ...
    '&:hover' : { '& $expandMoreIcon' : { opacity : 1,
      transition : "opacity 200ms ease-in", marginLeft: "0.2rem" } }
  },
  itemCategory : {
                      ...
                      <div className={classNames(classes.link)}>
                        ...
                        >
                          { (isDrawerCollapsed && children && (this.state.hoveredId === childId  || this.state.openItems.includes(childId))) ?
                            <ExpandMoreIcon
                              onClick={() => this.toggleItemCollapse(childId)}
                              className={classNames({ [classes.collapsed] : this.state.openItems.includes(childId) })} style={{ marginLeft: "0.3rem"}}
                            /> :
                            <ListItemIcon className={classes.listIcon}>
                              {icon}
                            </ListItemIcon>
