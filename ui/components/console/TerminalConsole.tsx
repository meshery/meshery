...
import { consoleSocketUrl, type ConsoleTarget } from 'lib/console/protocol';
import { useConsoleSocket } from 'lib/console/useConsoleSocket';
...
export interface TerminalConsoleProps {
  ...
  active?: boolean;
  /** Whether this console is the one on show, and so owns the panel's header. */
  focused?: boolean;
}
...
const TerminalConsole: React.FC<TerminalConsoleProps> = ({
  ...
  // a new console, not a reconfiguration of the running one.
  ...
  const url = useMemo(
    () =>
      active
        ? consoleSocketUrl(connectionId, 'terminal', {
            resource,
            namespace,
            name,
  ...
  const { status, capabilities, closed, send, resize } = useConsoleSocket({ url, onData });
  ...
  // geometry once the console attaches. fit() alone is not enough: it only
  ...
  useEffect(() => {
    ...
    if (closed.graceful) {
      writeln(`\r\n\x1b[2m— console ended (exit code ${closed.exitCode ?? 0}) —\x1b[0m`);
    } else {
      writeln(`\r\n\x1b[31m— console failed: ${closed.message ?? 'connection lost'} —\x1b[0m`);
    }
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <ConsoleToolbar>
        {/* Container and search render in the panel's header; see ConsoleControls. */}
        <ConsoleControls
