...
import { useGetConsoleCapabilitiesQuery } from '@meshery/schemas/mesheryApi';
...
import { ConsolePaneHost } from './console-styles';
...
export interface ConsolePanelProps {
  ...
  /**
   * Whether this console is the one on show. Distinct from `active`, which keeps a
   * hidden console's socket alive: a backgrounded tab still holds its shell, but
...
/**
 * Renders one console of the requested kind, once the server confirms the
 * target admits it.
 *
...
const ConsolePanel: React.FC<ConsolePanelProps> = ({
  ...
  if (error) {
    ...
    // offers consoles on resources the cluster has already deleted, and
    // "something went wrong" sends the user looking for a bug that isn't there.
    const described = describeConsoleError(error, target?.name);
    return (
  ...
  if (!supported) {
    return (
      <Alert severity="info" variant="outlined">
        {capabilities?.reason ?? `This resource does not support a ${kind} console.`}
      </Alert>
    );
  }

  const resolved: ConsoleTarget = {
    ...target,
    container: target.container || capabilities?.defaultContainer,
  };

  // The fullscreen control lives at the right of the console's own toolbar. It
  ...
  // wrapper element instead would move the console in the element tree, and
  // React would unmount and remount it — killing a live shell every time the
  // user hit fullscreen.
  return <ConsolePaneHost $fullScreen={fullScreen}>{pane}</ConsolePaneHost>;
};
