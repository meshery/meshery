...
/**
 * The consoles panel's host, in all three of its states.
 *
 ...
 * back. Hiding is `visibility`, not unmounting, because every console must stay
 * mounted while it is minimized or its shell dies with it.
 */
export const ConsoleShellHost = styled('div', {
  ...
  ({ $open, $floating, $minimized, $dockHeight }) => ({
    ...
    // height carved out of a page that has no consoles to show in it.
    ...(!$open && { display: 'none' }),
    ...($floating || $minimized
      ? {
          position: 'absolute',
 ...
 * `position: fixed` descendants, and a console's fullscreen state is exactly such
...
export const ConsoleHeaderBar = styled('div', {
  shouldForwardProp: (prop) => prop !== '$floating',
})<{ $floating: boolean }>(({ theme, $floating }) => ({
  display: 'flex',
  alignItems: 'center',
  gap: '0.25rem',
  // Tall enough for the console controls the focused console portals in.
  minHeight: '2.5rem',
...
/**
 * The header's controls slot: the focused console's container select and search
...
export const ConsoleTitle = styled('span')(({ theme }) => ({
  flex: '0 1 auto',
  minWidth: 0,
  overflow: 'hidden',
...
/** The panel's body: the tab strip and the console panes below it. */
 ...
 */
export const NavConsolesButton = styled('button', {
...
/**
 * The element a console renders into, in both normal and fullscreen states.
 *
 * Fullscreen is a style change on this one element rather than a different
 * wrapper: moving the console in the element tree would make React unmount and
 * remount it, killing the live shell or log stream underneath.
 */
export const ConsolePaneHost = styled('div', {
 ...
 */
export const ConsoleSurface = styled('div')(({ theme }) => ({
  position: 'relative',
  flex: 1,
  minHeight: 0,
  width: '100%',
...
/**
 * The single control row above a console.
 *
 * One row, not two: the console state used to live in a footer of its own, which
...
/**
 * Console state, inline in the toolbar. It yields its width first — the controls
...
/** The console tab strip: dense, so more tabs fit before it has to scroll. */
export const ConsoleTabs = styled(Tabs)(({ theme }) => ({
  minHeight: '2rem',
  borderBottom: `1px solid ${theme.palette.divider}`,
  '& .MuiTab-root': {
    minHeight: '2rem',
