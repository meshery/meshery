...
export type ConsolePanelMode = 'docked' | 'floating';
...
export interface ConsoleShellProps {
  ...
  onModeChange: (mode: ConsolePanelMode) => void;
  onMinimize: () => void;
  onClose: () => void;
  title: string;
...
/**
 * The chrome around the console panes: a title bar, dock/detach, minimize, close,
 * and the drag and resize behaviour behind them.
 *
 * Docked and floating are two styles of one element tree, never two trees. A
 * console is a live shell on the other end of a WebSocket, so React unmounting it
...
const ConsoleShell: React.FC<ConsoleShellProps> = ({
  ...
  // State, not a ref: the consoles below portal into this element, so they must
  ...
  const startDrag = (event: React.PointerEvent) => {
    ...
    // the focused console's container select and search box. Dragging the panel
  ...
  return (
    ...
    >
      ...
      >
        <ConsoleHeaderBar $floating={floating} onPointerDown={startDrag}>
          <ConsoleTitle>{title}</ConsoleTitle>

          {/* The focused console portals its container select and search in here. */}
          <ConsoleHeaderActions ref={setHeaderSlot} data-no-drag />

          <Tooltip title={floating ? 'Dock to bottom' : 'Detach'}>
            <IconButton
              size="small"
              aria-label={floating ? 'Dock consoles panel' : 'Detach consoles panel'}
          ...
          <Tooltip title="Minimize">
            <IconButton size="small" aria-label="Minimize consoles panel" onClick={onMinimize}>
              <ExpandMoreIcon width={16} height={16} fill="currentColor" />
            </IconButton>
          </Tooltip>

          <Tooltip title="Close all consoles">
            <IconButton size="small" aria-label="Close all consoles" onClick={onClose}>
        ...
        <ConsoleBody>
          <ConsoleHeaderSlotContext.Provider value={headerSlot}>
            {tabs}
            {children}
