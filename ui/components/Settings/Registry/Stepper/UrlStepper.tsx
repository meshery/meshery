...
const UrlStepper = React.memo(({ handleClose }: UrlStepperProps) => {
  ...
  const [modelSource, setModelSource] = React.useState<string>('');
  const [modelName, setModelName] = React.useState<string>('');
  const [modelDisplayName, setModelDisplayName] = React.useState<string>('');
  const [modelCategory, setModelCategory] = React.useState<string>(
    CategoryDefinitionV1Beta1Schema.properties.name.default,
  );
  const [modelSubcategory, setModelSubcategory] = React.useState<string>(
  ...
  const [logoLightThemePath, setLogoLightThemePath] = React.useState<string>('');
  const [logoDarkThemePath, setLogoDarkThemePath] = React.useState<string>('');
  ...
  const SvgLogoDisplay = ({ svgContent }: { svgContent?: string }) => {
    if (!svgContent) {
      return (
