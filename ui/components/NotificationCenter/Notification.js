...
function Notification(props) {
  const {
    classes, className,expand,onMarkAsRead,onDeleteEvent
  } = props;

  const {
    summary,event_type,
    details,  probable_cause,
    suggested_remediation,  error_code,
    component_type, component_name,
    timestamp,status } = props.event

  const variant = event_type.type
  ...
  useEffect(() => {
    ...
    if (component_type === "adapter") {
      if (summary?.includes("mesh installed")) {
        setSocialMessage(generateMsgForMesh(component_name[0].toUpperCase() + component_name.substring(1).toLowerCase()))
        return
      }
      if (summary.includes("application installed")) {
        const name = summary?.split(" ")[0];
        setSocialMessage(generateMsgForSampleApp(name[0].toUpperCase() + name.substring(1).toLowerCase()))
        return
      }
      setSocialMessage(getDefaultMessage(summary))
      return
    }
    if (component_type === "core" && summary?.includes("deployed")) {
      const designName = summary?.split(":")[1]
          ...
          <Grid container direction="row" justify="space-between" alignItems="center" wrap="nowrap">
            <Typography variant="subtitle2">
              <div style={{ display : "flex", alignItems : "center" }}>
                ...
                <div style={{ padding : "0.5rem" }}>
                  ...
                  <Typography className={classes.timestamp}> {formatTimestamp(timestamp)} </Typography>
                </div>
              </div>
            </Typography>
            ...
            <Grid container item xs={4} className={classes.icons} justify="flex-end">
              ...
              {status == NOTIFICATION_STATUS.VIEWED  &&
                <IconButton className={classes.expand} onClick={handleClose}>
                  <DeleteIcon
          ...
          {variant === EVENT_TYPES.ERROR.type  &&
            <>
              ...
              <Paper className={classes.collapse} square variant="outlined" elevation={0}>
                ...
                <a href={`${ERROR_DOC_LINK}#meshery-${component_type}-for-meshery-${component_name.toLowerCase()}`} target="_blank" rel="referrer noreferrer"> {error_code} </a>
              </Paper>}
            </>
          }
        </Collapse>
