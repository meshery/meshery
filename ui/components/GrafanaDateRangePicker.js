...
import MesheryDateTimePicker from "./MesheryDateTimePicker.js";

const styles = (theme) => ({
  rangeDialog: {
    // width: window.innerWidth * 0.7,
  },
  rangeDialogRow: { display: "flex", gap: "4rem", "&>*": { flex: "1" } },
  dateTimePicker: { marginTop: theme.spacing(2), marginBottom: theme.spacing(1) },
  innerGrid: {
              ...
              <DialogContentText className={classes.rangeDialog}>
                <Grid container>
                  <Grid item xs={12}>
                    ...
                    <div className={classes.rangeDialogRow}>
                      <FormControlLabel
                        control={<Switch checked={liveTail} color="primary" onChange={this.handleChange("liveTail")} />}
                        label="Live tail"
                      />
                      <TextField
                        select
                    ...
                    <Grid container spacing={0}>
                      {quickRanges.map((qr) => (
                        <Grid item xs={12} sm={3} className={classes.timeList}>
                          {qr.map((q) => (
                            <Button variant="text" onClick={this.setRange(q)}>
                              {q}
                            </Button>
                          ))}
                        </Grid>
