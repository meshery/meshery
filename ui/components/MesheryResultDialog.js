...
import { NoSsr, Grid, Table, TableRow, TableCell, TableBody, Typography } from '@mui/material';
...
function MesheryResultDialog({ rowData, close }) {
  ...
  const createTableRow = (key, val) => (
    <TableRow>
      <TableCell align="right" component="th" scope="row" sx={{ borderBottom: 'none' }}>
        {key}:
      </TableCell>
      <TableCell sx={{ borderBottom: 'none' }}>{val}</TableCell>
    </TableRow>
  );
        ...
        <TableBody>
          ...
          {kubernetes.nodes.map((node, ind) => (
            <NoSsr key={node.uniqueID}>
              <TableRow>
                <TableCell colSpan={2} align="center" sx={{ borderBottom: 'none' }}>
                  <strong>Node {ind + 1}</strong>
                </TableCell>
              </TableRow>
            ...
            {meshes.map((mesh, ind) => (
              <NoSsr key={mesh}>
                {meshes.length > 1 && (
                  <TableRow>
                    <TableCell colSpan={2} align="center" sx={{ borderBottom: 'none' }}>
                      <strong>Service Mesh {ind + 1}</strong>
                    </TableCell>
                  </TableRow>
                )}
  ...
  const renderLoadProfile = (rowData) => {
    ...
    if (rowData.runner_results) {
      ...
      contents = [
        ...
        createTableRow('Actual Duration', (rowData.runner_results.ActualDuration / 1e9).toFixed(1)),
          ...
          content={
            <div>
              <Grid container spacing={1}>
                {rowData?.runner_results?.kubernetes && (
                ...
                {rowData?.runner_results?.['detected-meshes'] && (
                  <Grid item xs={12} sm={6}>
