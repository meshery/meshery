  ...
  useEffect(() => {
    ...
    if (componentType === 'adapter') {
      if (message?.includes('mesh installed')) {
        setSocialMessage(
          generateMsgForMesh(
            componentName[0].toUpperCase() + componentName.substring(1).toLowerCase(),
          ),
        );
        return;
      }
      if (message.includes('application installed')) {
        ...
        setSocialMessage(
          generateMsgForSampleApp(name[0].toUpperCase() + name.substring(1).toLowerCase()),
        );
        return;
      }
  ...
  return (
    <SnackbarContent className={classes.snackbarContent}>
      <Card
        className={
          highlight
            ? classNames(classes.card, classes[variant], className, classes.snackbarContentBorder)
            : classNames(classes.card, classes[variant], className)
        }
              ...
              >
                <ExpandMoreIcon
                  className={classNames({
                    [classes.iconColor]: !cardHover,
                    [classes[variant]]: cardHover,
                  })}
                />
              </IconButton>
                ...
                >
                  <ReplyIcon
                    className={classNames(
                      classes.share,
                      classes.iconColor,
                      classes[variantHoverColor[variant]],
                    )}
