...
const styles = (theme) => ({
  ...
  chartTitleGraf : { textAlign : "center",
...
class MesheryPerformanceComponent extends React.Component {
    ...
    dataFetch(
      ...
      (result) => {
        if (typeof result !== "undefined") {
          ...
          if (generateNotif) {
            this.props.enqueueSnackbar("Performance Profile Successfully Created!", { variant : "success",
      ...
      (err) => {
        console.error(err);
        this.props.updateProgress({ showProgress : false });
        this.props.enqueueSnackbar("Failed to create performance profile", { variant : "error",
          autoHideDuration : 2000,
          action : (key) => (
  ...
  static generateTestName = (name, meshName) => {
    if (!name || name.trim() === "") {
      const mesh = meshName === "" || meshName === "None"
        ? "No mesh"
        : meshName;
  ...
  submitLoadTest = (id) => {
    ...
    const data = {
      ...
      mesh : meshName === "" || meshName === "None"
        ? ""
        : meshName, // to prevent None from getting to the DB
      url,
      qps,
      c,
      t : t1,
  ...
  handleSuccess() {
    ...
    return (result) => {
      ...
      if (typeof result !== "undefined" && typeof result.runner_results !== "undefined") {
        self.props.enqueueSnackbar("Successfully fetched the data.", { variant : "success",
          autoHideDuration : 2000,
          action : (key) => (
  ...
  async startEventStream(url) {
    ...
    this.props.enqueueSnackbar("Load test has been successfully submitted", { variant : "info",
  ...
  handleEvents() {
    ...
    return (e) => {
      ...
      switch (data.status) {
        case "info":
          self.props.enqueueSnackbar(data.message, { variant : "info",
            autoHideDuration : 1000,
            action : (key) => (
              <IconButton key="close" aria-label="Close" color="inherit" onClick={() => self.props.closeSnackbar(key)}>
                <CloseIcon />
    ...
    dataFetch(
      ...
      (result) => {
        if (
          ...
          self.props.updateStaticPrometheusBoardConfig({ staticPrometheusBoardConfig : result, // will contain both the cluster and node keys for the respective boards
          });
  ...
  handleError(msg) {
    ...
    return (error) => {
      ...
      self.props.enqueueSnackbar(finalMsg, { variant : "error",
        action : (key) => (
          <IconButton key="close" aria-label="Close" color="inherit" onClick={() => self.props.closeSnackbar(key)}>
            <CloseIcon />
          </IconButton>
            ...
            <Grid container spacing={1}>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  error={urlError}
                  helperText={urlError
                    ? "Please enter a valid URL along with protocol"
                    : ""}
                  margin="normal"
                  variant="outlined"
