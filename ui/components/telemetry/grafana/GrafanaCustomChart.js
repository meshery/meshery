...
const CachedIconCardHeaderIcon = styled(CachedIcon)(({ theme }) => ({
  fontSize : theme.spacing(2),
}));

const OpenInNewIconCardHeaderIcon = styled(OpenInNewIcon)(({ theme }) => ({
  fontSize : theme.spacing(2),
}));
...
const WarningIconError = styled(WarningIcon)(() => ({
  color : "#D32F2F",
}));
...
class GrafanaCustomChart extends Component {
  ...
  getData = async (ind, target) => {
    ...
    if (typeof panel.datasource !== "string") {
      ds =
        panel?.datasource?.type.charAt(0).toUpperCase() +
        panel.datasource.type.substring(1);
    } else {
      ds =
        panel?.datasource?.charAt(0).toUpperCase() +
        panel.datasource.substring(1);
    }
  ...
  render() {
    ...
    if (this.panelType === "gauge") {
      ...
      mainChart = (
        <div>
          <div style={{ width : "100%" }} ref={(ch) => (self.chartRef = ch)} />
        </div>
      );
    }
