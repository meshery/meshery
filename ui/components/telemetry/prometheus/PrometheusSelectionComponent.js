...
class PrometheusSelectionComponent extends Component {
      ...
      (result) => {
        ...
        if (typeof result !== 'undefined') {
          this.setState({
            ...
            templateVars:
              result.template_vars && result.template_vars.length > 0 ? result.template_vars : [],
            templateVarOptions: [],
  ...
  addSelectedBoardPanelConfig = () => {
    const { grafanaBoardObject, templateVars, selectedTemplateVars, selectedPanels, panels } =
      this.state;
    const boardConfig = {};
                    ...
                    renderValue: (selected) => (
                      <PanelChipsContainer>
                        {selected.map((value) => {
                          ...
                          return <Chip key={`pl_--_${panelId}`} label={selVal} className="chip" />;
                        })}
                      </PanelChipsContainer>
                    ),
                  }}
