...
const Namespaces = ({ classes, updateProgress, k8sConfig }) => {
  const ALL_NAMESPACE = 'all';
  const SINGLE_NAMESPACE = 'single';
  ...
  const getMeshsyncResources = (page, pageSize, search, sortOrder) => {
    ...
    dataFetch(
      `/api/system/meshsync/resources?kind=Namespace&status=true&annotations=true&labels=true&spec=true&page=${page}&pagesize=${pageSize}&search=${encodeURIComponent(
        search,
          ...
          return (
            <>
              <div
                ...
                onClick={() => swtichView(SINGLE_NAMESPACE, meshSyncResources[tableMeta.rowIndex])}
              >
                {value}
              </div>
            </>
