...
/**
 * The sessions panel's dock state, mirrored outside React.
 *
 * The restore affordance for a minimized panel lives at the foot of the
 * Navigator, and the Navigator is mounted *above* `SessionsProvider` in the tree
...
export interface SessionsDockState {
  /** How many sessions are open. Zero means there is nothing to restore. */
  count: number;
  /** Whether the panel is currently minimized out of view. */
  minimized: boolean;
}

const NO_SESSIONS: SessionsDockState = { count: 0, minimized: false };

let state: SessionsDockState = NO_SESSIONS;
 ...
 * render.
 */
export const publishDockState = (next: SessionsDockState) => {
...
export const bindDock = (onRestore: () => void) => {
  ...
  return () => {
    ...
    restore = null;
    publishDockState(NO_SESSIONS);
  };
};

export const restoreSessions = () => restore?.();

export const useSessionsDock = (): SessionsDockState =>
