...
import { bindDock, publishDockState, restoreSessions, useSessionsDock } from '../dock-store';
...
describe('dock-store', () => {
  ...
  it('publishes dock state to subscribers', () => {
    const { result } = renderHook(() => useSessionsDock());
  ...
  it('clears published state when the provider unmounts', () => {
    ...
    expect(restoreSessions()).toBeUndefined();
  });
