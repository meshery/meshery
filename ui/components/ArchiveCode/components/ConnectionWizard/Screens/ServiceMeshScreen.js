...
import ServiceMeshConfig from '../ConfigComponents/ServiceMesh.js';
import ServiceCard from '../ServiceCard.js';
import { Grid } from '@material-ui/core';
import VerticalCarousel from '../../../../VerticalCarousel/VerticalCarousel.js';
import ServiceMeshDataPanel from '../DataPanels/ServiceMesh.js';
import { createRef, useEffect, useState } from 'react';
import { fetchAvailableAdapters } from '../../../../../utils/helpers/serviceMeshes.js';
  ...
  const itemsToBeRendered = serviceMeshComponents.map((comp) => {
    return (
      <ServiceCard
        key={comp.uniqueID}
        serviceInfo={comp}
        isConnected={isAdapterActive(comp.adapterInfo.value)}
      />
    );
  });
  ...
  return (
    <Grid xs={12} container justify="center" alignItems="flex-start">
      ...
      <Grid item lg={6} sm={12} md={12} container justify="center" style={{ paddingRight: '1rem' }}>
        <ServiceMeshDataPanel
          adapterInfo={serviceMeshComponents[activeIndex]?.adapterInfo}
          isActive={isAdapterActive(serviceMeshComponents[activeIndex]?.adapterInfo.value)}
        />
      </Grid>
    </Grid>
  );
