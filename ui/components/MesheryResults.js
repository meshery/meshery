...
class MesheryResults extends Component {
  ...
  render() {
    ...
    const { results, page, count, pageSize, selectedRowData } = this.state;
    const self = this;
    ...
    results.forEach((record) => {
      ...
      if (
        record.runner_results.DurationHistogram &&
        record.runner_results.DurationHistogram.Percentiles
      ) {
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta) => (
            <IconButton
              ...
              color="inherit"
              onClick={() =>
                self.setState({ selectedRowData: self.state.results[tableMeta.rowIndex] })
              }
            >
              <MoreHorizIcon />
            </IconButton>
