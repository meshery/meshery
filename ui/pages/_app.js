...
import { loadPersistedState, store } from '../store';
...
import { PersistedStateProvider } from '@/store/redux-persist';
...
class MesheryApp extends App {
  ...
  loadWorkspace = async (orgId) => {
    const currentWorkspace = this.props.rtkSelector(getCurrentWorkspace);
    if (currentWorkspace && currentWorkspace !== 'undefined') {
      let workspace = JSON.parse(currentWorkspace);
      this.props.rtkDispatch(selectCurrentWorkspace(workspace));
    } else {
      dataFetch(
...
const MesheryWithRedux = (props) => {
  ...
  return <Meshery {...props} rtkDispatch={rtkDispatch} rtkSelector={useSelectorRtk} />;
};
    ...
    <Provider store={store} context={RTKContext}>
      <Provider store={props.store} context={LegacyStoreContext}>
        <Provider store={props.store}>
          ...
          <MuiPickersUtilsProvider utils={MomentUtils}>
            <PersistedStateProvider loadPersistedState={loadPersistedState}>
