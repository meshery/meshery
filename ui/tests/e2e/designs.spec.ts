...
test.describe.skip('Design Page Tests', () => {
  ...
  DESIGN_TYPES.forEach(({ type }) => {
    test(`displays ${type} design card correctly`, async ({ provider }: { provider: string }) => {
      test.skip(
  ...
  test('deploys a published design to a connected cluster', async ({ page }: { page: Page }) => {
    await page.route('**/api/environments?page=0&pagesize=all&orgID=*', mockEnvironmentsApi);
    await page.route('**/api/environments/*/connections?page=0&pagesize=all', mockConnectionsApi);
    await page.route('**/api/pattern/deploy?contexts=*', async (route) => await route.fulfill());
    await designPage.applyVisibilityFilter('published');
    const card = await designPage.getFirstCardByVisibilityBadge('published');
    const cardElements = designPage.getCardElements(card, 'published');
    await cardElements.actionToggleBtn.click();
