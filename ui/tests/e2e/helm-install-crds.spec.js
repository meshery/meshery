...
test.describe('Meshery Helm CRD Installation Tests', () => {
  ...
  test('should not install Meshery CRDs when installCRDs=false', async () => {
    ...
    try {
      ...
      console.log(
        `Installing Meshery release '${releaseName}' in namespace '${namespace}' with installCRDs=false...`,
      );
      execShellCommand(
        `helm install ${releaseName} meshery/meshery --namespace ${namespace} --create-namespace --set installCRDs=false`,
      );
      ...
      console.log(
        `Cleaning up: Uninstalling ${releaseName} and deleting namespace ${namespace}...`,
      );
      try {
        execShellCommand(`helm uninstall ${releaseName} --namespace ${namespace} || true`);
      } catch (e) {
        console.warn(
          `Warning: Uninstall failed for ${releaseName} (may not exist or other issue): ${e.message}`,
        );
      }
      try {
        // Use --force and --grace-period=0 for immediate deletion in tests, and --timeout for robustness
        execShellCommand(
          `kubectl delete namespace ${namespace} --timeout=2m --force --grace-period=0 || true`,
        );
      } catch (e) {
        console.warn(
          `Warning: Namespace deletion failed for ${namespace} (may not exist or other issue): ${e.message}`,
        );
      }
    }
  });
  ...
  test('should install Meshery CRDs by default', async () => {
    ...
    try {
      ...
      console.log(`--- Test: ${test.info().title} ---`);
      console.log(
        `Installing Meshery release '${releaseName}' in namespace '${namespace}' with default CRD installation...`,
      );
      execShellCommand(
        `helm install ${releaseName} meshery/meshery --namespace ${namespace} --create-namespace`,
      );
      ...
      for (const crd of mesheryCRDs) {
        ...
        expect(
          crdFound,
          `CRD '${crd}' should be present but was not found after retries.`,
        ).toBeTruthy();
      }
    } finally {
