...
test.describe('Relationship Evaluation', { tag: '@relationship' }, () => {
    ...
    for (const { id, name } of DESIGNS_TO_TEST) {
        test(`should identify relationships for ${name}`, async ({ request }, testInfo) => {
            ...
            const designResponse = await request.get(`${ENV.REMOTE_PROVIDER_URL}/api/content/patterns/${id}`);
            ...
            for (const expectedRel of design.relationships) {
                if (expectedRel.status !== "approved" || !expectedRel.selectors || expectedRel?.metadata?.isAnnotation || expectedRel.subType === 'annotation') {
                    continue;
                }
