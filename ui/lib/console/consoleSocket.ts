/**
 * Low-level client for a console WebSocket.
 *
 ...
 * or React. That keeps it usable by a future console kind, and testable without
 * a DOM.
 *
...
/** Why a console ended. */
export interface ConsoleClosed {
  /** True when the console ended as intended: command exited, or logs hit EOF. */
  graceful: boolean;
...
export interface ConsoleSocketHandlers {
  ...
  onReady?: (capabilities: ConsoleCapabilities) => void;
  /** Fired for every binary frame: stdout or log bytes. */
  onData?: (chunk: Uint8Array) => void;
  /** Fired once, when the console ends for any reason. */
  onClose?: (info: ConsoleClosed) => void;
}

export class ConsoleSocket {
  private socket: WebSocket;
  private handlers: ConsoleSocketHandlers;
  ...
  constructor(url: string, handlers: ConsoleSocketHandlers = {}) {
  ...
  /** Closes the console. Safe to call more than once. */
  close(): void {
    ...
    if (
      ...
      this.socket.close(1000, 'client closed the console');
    }
  }
  ...
  private onControl(payload: string): void {
    ...
    try {
      ...
      // A frame we cannot parse is not worth killing a live console over.
      return;
    }
    ...
    switch (message.type) {
      ...
      case 'eof':
        ...
        // there is exactly one close notification per console.
  ...
  private onSocketError = (): void => {
    // `error` is always followed by `close`, which is where the console's
   ...
   * carries neither.
   */
  private describeClose(event: CloseEvent): ConsoleClosed {
    ...
    if (
      this.closedByClient ||
      event.code === CLOSE_CONSOLE_ENDED ||
      event.code === 1000 ||
      event.code === 1005
    ) {
      return { graceful: true };
    ...
    return {
      graceful: false,
      message:
        event.code === CLOSE_CONSOLE_FAILED && event.reason
          ? event.reason
          : 'The connection to the console was lost.',
    };
  }
}
