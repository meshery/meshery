...
/**
 * Captures the handlers each ConsoleSocket was constructed with, so a test can
...
describe('useConsoleSocket', () => {
  it('drops data from a socket it has already torn down', () => {
    const onData = vi.fn();
    const { rerender } = renderHook(({ url }) => useConsoleSocket({ url, onData }), {
  ...
  it('keeps the live socket’s status after a superseded socket closes', () => {
    const { result, rerender } = renderHook(({ url }) => useConsoleSocket({ url }), {
      initialProps: { url: 'ws://host/a' },
    });
  ...
  it('holds the socket shut when there is no url', () => {
    renderHook(() => useConsoleSocket({ url: null }));
