...
import { ConsoleSocket, type ConsoleClosed } from '../consoleSocket';
...
const openSocket = (handlers: { onClose?: (info: ConsoleClosed) => void } = {}) => {
  const socket = new ConsoleSocket('ws://localhost/console', handlers);
  return { socket, fake: FakeWebSocket.instances.at(-1)! };
};

describe('ConsoleSocket close reporting', () => {
  it('reports a client-initiated close of a CONNECTING socket as graceful', () => {
    ...
    let closed: ConsoleClosed | undefined;
  ...
  it('still reports an unexpected close as a lost connection', () => {
    ...
    expect(closed?.message).toContain('connection to the console was lost');
  });
