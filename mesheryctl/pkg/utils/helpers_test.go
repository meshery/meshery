...
func TestDisplayCountOnly(t *testing.T) {
	type args struct {
		component          string
		count              int64
	...
	tests := []struct {
		...
		{
			name: "display count with list --count return false",
			args: args{
				component:          "test",
				count:              111,
		...
		{
			name: "display count list --count --page x return true",
			args: args{
				component:          "test",
				count:              111,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := DisplayCountOnly(tt.args.component, tt.args.count, tt.args.paginationRequired); got != tt.want {
				t.Errorf("DisplayCountOnly() = %v, want %v", got, tt.want)
			}
		})
	}
}
