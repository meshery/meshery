...
func TestDesignView(t *testing.T) {
	...
	tests := []struct {
		...
		{
			Name:             "Fetch Design View by ID (Without Verbose)",
			Args:             []string{"view", "design"},
			ExpectedResponse: "view.design.output.golden",
			Fixture:          "view.design.api.response.golden",
			URL:              testContext.BaseURL + "/api/pattern/" + "design",
			Token:            filepath.Join(fixturesDir, "token.golden"),
			ExpectError:      false,
		},
		{
			Name:             "Fetch Design View by ID (With Verbose)",
			Args:             []string{"view", "design", "--verbose"},
			ExpectedResponse: "view.design.verbose.output.golden",
			Fixture:          "view.design.api.response.golden",
			URL:              testContext.BaseURL + "/api/pattern/" + "design",
			Token:            filepath.Join(fixturesDir, "token.golden"),
			ExpectError:      false,
