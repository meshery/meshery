...
func TestDeployCmd(t *testing.T) {
	...
	tests := []utils.MesheryMultiURLCommamdTest{
		...
		{
			Name:             "given design whose components all report success when design deploy then design is deployed",
			Args:             []string{"deploy", "-f", filepath.Join(fixturesDir, "sampleDesign.golden"), "-s", "Kubernetes Manifest"},
			ExpectedResponse: "deploy.summary.output.golden",
			URLs: []utils.MockURL{
				{
					Method:       "GET",
					URL:          testContext.BaseURL + "/api/pattern/types",
				...
				},
				{
					Method:       "POST",
					URL:          testContext.BaseURL + "/api/pattern/deploy",
					Response:     "deploy.designdeploy.summary.response.golden",
					ResponseCode: 200,
				},
			},
			ExpectError: false,
		...
		{
			Name:             "given design with a failed component when design deploy then partial failure error is returned",
			Args:             []string{"deploy", "-f", filepath.Join(fixturesDir, "sampleDesign.golden"), "-s", "Kubernetes Manifest"},
			ExpectedResponse: "",
			URLs: []utils.MockURL{
				{
				...
				},
				{
					Method:       "POST",
					URL:          testContext.BaseURL + "/api/pattern/deploy",
					Response:     "deploy.designdeploy.partial.response.golden",
					ResponseCode: 200,
				},
			},
			ExpectError:    true,
			IsOutputGolden: false,
			ExpectedError:  ErrDesignPartialFailure("deployed", []string{"myapp"}),
		},
		{
