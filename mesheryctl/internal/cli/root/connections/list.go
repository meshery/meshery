...
func init() {
	...
	listConnectionsCmd.Flags().IntVarP(&connectionListFlagsProvided.pageSize, "pagesize", "s", 10, "Number of connections per page")
	listConnectionsCmd.Flags().StringSliceVarP(&connectionListFlagsProvided.status, "status", "t", []string{}, "Filter connections by status")
}
