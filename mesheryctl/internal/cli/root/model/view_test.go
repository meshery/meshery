...
func TestViewModel(t *testing.T) {
	...
	tests := []utils.MesheryListCommandTest{
		...
		},
		{
			...
			Fixture:          "list.model.empty.api.response.golden",
			ExpectedResponse: "view.model.empty.ouput.golden",
			ExpectError:      false,
			IsOutputGolden:   true,
		},
		{
			...
			ExpectedResponse: "view.model.ouput.golden",
			ExpectError:      false,
			IsOutputGolden:   true,
		},
		{
			...
			URL:            fmt.Sprintf("/%s?search=%s&pagesize=all", modelsApiPath, url.QueryEscape(modelName)),
			Fixture:        "list.model.empty.api.response.golden",
			ExpectError:    true,
		...
		},
		{
			...
			URL:              fmt.Sprintf("/%s?search=%s&pagesize=all", modelsApiPath, url.QueryEscape(modelName)),
			Fixture:          "list.model.api.response.golden",
			ExpectedResponse: "view.json.api.response.golden",
			ExpectError:      false,
			IsOutputGolden:   true,
		},
	}
