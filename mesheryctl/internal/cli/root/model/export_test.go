...
func TestExportModel(t *testing.T) {
	...
	tests := []struct {
		...
		{
			Name:             "Export model with discarded components",
			Args:             []string{"export", "amd-gpu", "--discard-components"},
			URL:              fmt.Sprintf("%s?name=amd-gpu&output_format=yaml&file_type=oci&components=false&relationships=true&pagesize=all", apiURL),
			Fixture:          "export.model.api.response.golden",
			ExpectedResponse: "export.model.default.output.golden",
			ExpectError:      false,
			HttpCode:         200,
		...
		{
			Name:             "Export model with discarded relationships",
			Args:             []string{"export", "amd-gpu", "--discard-relationships"},
			URL:              fmt.Sprintf("%s?name=amd-gpu&output_format=yaml&file_type=oci&components=true&relationships=false&pagesize=all", apiURL),
			Fixture:          "export.model.api.response.golden",
			ExpectedResponse: "export.model.default.output.golden",
			ExpectError:      false,
			HttpCode:         200,
