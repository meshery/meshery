...
func registerModel(data []byte, componentData []byte, relationshipData []byte, filename string, dataType string, sourceURI string, register bool) error {
	...
	default:
		if data != nil {
			var modelDef modelv1beta2.ModelDefinition
			if err := encoding.Unmarshal(data, &modelDef); err == nil && (strings.HasPrefix(modelDef.SchemaVersion, "models.meshery.io/") || modelDef.Category.Name != "") {
				// Mapped successfully from standard ModelDefinition generated by model init
				importRequest.ImportBody.Model.Model = modelDef.Name
				importRequest.ImportBody.Model.ModelDisplayName = modelDef.DisplayName

				// Prefer explicit registrant name, fallback to kind if empty
				if modelDef.Registrant.Name != "" {
					importRequest.ImportBody.Model.Registrant = modelDef.Registrant.Name
				} else {
					importRequest.ImportBody.Model.Registrant = modelDef.Registrant.Kind
				}

				importRequest.ImportBody.Model.Category = category.CategoryDefinitionName(modelDef.Category.Name)
				importRequest.ImportBody.Model.SubCategory = subcategory.SubCategoryDefinition(modelDef.SubCategory)

				if modelDef.Metadata != nil {
					if modelDef.Metadata.IsAnnotation != nil {
						importRequest.ImportBody.Model.IsAnnotation = *modelDef.Metadata.IsAnnotation
					}
					if modelDef.Metadata.PrimaryColor != nil {
						importRequest.ImportBody.Model.PrimaryColor = *modelDef.Metadata.PrimaryColor
					}
					if modelDef.Metadata.SecondaryColor != nil {
						importRequest.ImportBody.Model.SecondaryColor = *modelDef.Metadata.SecondaryColor
					}
					importRequest.ImportBody.Model.SvgColor = modelDef.Metadata.SvgColor
					importRequest.ImportBody.Model.SvgWhite = modelDef.Metadata.SvgWhite
					if modelDef.Metadata.SvgComplete != nil {
						importRequest.ImportBody.Model.SvgComplete = *modelDef.Metadata.SvgComplete
					}
					if modelDef.Metadata.Shape != nil {
						importRequest.ImportBody.Model.Shape = string(*modelDef.Metadata.Shape)
					}
				}
			} else {
				// Fallback to legacy flattened template schema
