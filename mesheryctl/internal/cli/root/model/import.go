...
// scaffoldToModel converts a model scaffold template into a schemav1beta1.Model
// suitable for use in an ImportRequest. It accepts both the simple flat template
// format (fixtures/template.json) and the full canonical ModelDefinition format
// generated by "mesheryctl model init".
//
// By unmarshaling into typed structs rather than map[string]interface{}, this
// function avoids brittle string-key guessing. The Go compiler enforces field
// names, so any upstream schema rename is caught at compile time rather than
// silently dropping data at runtime.
func scaffoldToModel(data []byte) (schemav1beta1.Model, error) {
	var result schemav1beta1.Model

	// First, try the canonical ModelDefinition format (generated by model init).
	// This is a typed unmarshal — if the template is a full ModelDefinition,
	// all fields are extracted cleanly without any manual key-name guessing.
	var modelDef schemav1beta1model.ModelDefinition
	if err := encoding.Unmarshal(data, &modelDef); err == nil && modelDef.Name != "" {
		// Model name: canonical schema uses "name"
		result.Model = modelDef.Name
		// Display name: canonical schema uses "displayName"
		result.ModelDisplayName = modelDef.DisplayName
		// Registrant: canonical schema stores as a Connection object; extract its Kind
		result.Registrant = string(modelDef.Registrant.Kind)
		// Category: canonical schema stores as a CategoryDefinition object; extract its Name
		result.Category = modelDef.Category.Name
		// SubCategory: extract the SubCategoryDefinition name string
		result.SubCategory = modelDef.SubCategory
		// Metadata fields: nested under ModelDefinition.Metadata
		if modelDef.Metadata != nil {
			...
			// Bug fix: shape was previously not copied — users who set
			// "shape" in their model.json would silently get the default shape.
	...
	// Fall back to the simple flat template format (fixtures/template.json).
	// This format already matches schemav1beta1.Model key-for-key, so a
	// direct unmarshal is sufficient — with one exception: the flat template
	// uses "displayName" while schemav1beta1.Model expects "modelDisplayName".
	// We resolve this alias after unmarshaling.
	if err := encoding.Unmarshal(data, &result); err != nil {
		return result, err
	}
	// Bug fix: the flat template uses "displayName" but schemav1beta1.Model
	// has json tag "modelDisplayName". The unmarshal above silently drops
	// "displayName" because the tags don't match. Resolve the alias here.
