	...
	case "docker":
		...
		for _, v := range currCtx.GetComponents() {
			if utils.Services[v].Image == "" {
				utils.Log.Fatalf("Invalid component specified %s", v)
			}
		...
		for k, v := range AllowedServices {
			...
			if err := utils.ViperCompose.WriteConfig(); err != nil {
				...
				utils.Log.Errorf("Encountered an error while adding `%s` service to Docker Compose file. Verify permission to write to `.meshery/meshery.yaml` file.", k)
			}
		}
		...
		if !endpointReady {
			utils.Log.Warnf("Warning: Meshery endpoint is not yet accessible. The server may still be initializing.")
