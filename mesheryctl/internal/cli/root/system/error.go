...
func ErrHealthCheckFailed(err error) error {
    return errors.New(ErrHealthCheckFailedCode, errors.Alert, [] string {
        ...
        "Health checks execution failed",
    }, [] string {
        "Health checks execution should passed to start Meshery server successfully",
    })
}
...
func ErrDownloadFile(err error, obj string) error {
    return errors.New(ErrDownloadFileCode, errors.Alert, [] string {
        ...
        "Make sure docker-compose or manifest file is downloaded successfully",
    })
}
...
func ErrUnmarshal(err error, obj string) error {
    return errors.New(ErrUnmarshalCode, errors.Alert, [] string {
        ...
        "Either the JSON response or the Response is distorted",
    })
}
...
func ErrRetrievingCurrentContext(err error) error {
    return errors.New(ErrRetrievingCurrentContextCode, errors.Alert, [] string {
        ...
        "current context is not retrieved successfully",
    }, [] string {
        "Verify current context is retrieved successfully and valid",
    })
}
...
func ErrRunPortForward(err error) error {
    return errors.New(
        ...
        errors.Fatal, [] string {
            ...
            err.Error(), "Error running port-forward for Meshery",
        }, [] string {
            "Meshery pod is not in running phase", "mesheryctl can't connect to kubernetes with client-go",
        }, [] string {
            "Make sure Meshery pod exists and is in running state",
...
func ErrFailedGetEphemeralPort(err error) error {
    return errors.New(
        ...
        errors.Fatal, [] string {
            "Failed to get a free port",
        }, [] string {
            err.Error(), "Failed to start port-forwarding",
        },
        nil, nil,
    )
}
