...
func TestExperimentalSearch(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			Args:             []string{"relationship", "search", "--model", "kubernetes"},
			URL:              testContext.BaseURL + "/api/meshmodels/models/kubernetes",
			Fixture:          "search.exp.relationship.api.response.golden",
			ExpectedResponse: "search.exp.relationship.output.golden",
