...
func TestDeleteWorkspace(t *testing.T) {
	...
	tests := []utils.MesheryCommandTest{
		...
		{
			Name:             "Delete workspace with too many arguments",
			Args:             []string{"delete", workspaceId, workspaceId},
			HttpMethod:       "DELETE",
			HttpStatusCode:   200,
			URL:              "/api/workspaces",
			Fixture:          "",
			...
			ExpectError:      true,
			IsOutputGolden:   false,
			ExpectedError:    utils.ErrInvalidArgument(errors.New("too many arguments\n\nUsage: mesheryctl workspace delete [workspaceId]\nRun 'mesheryctl workspace delete --help' to see detailed help message")),
		},
		{
			Name:             "Delete workspace not found (404)",
			Args:             []string{"delete", workspaceId},
			HttpMethod:       "DELETE",
			HttpStatusCode:   404,
			URL:              fmt.Sprintf("/api/workspaces/%s", workspaceId),
			Fixture:          "delete.workspace.notfound.api.response.golden",
			ExpectedResponse: "delete.workspace.notfound.output.golden",
			ExpectError:      false,
		},
		{
