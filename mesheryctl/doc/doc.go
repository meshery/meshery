...
const commandTrackerHeader = `# This file is auto-generated by mesheryctl/doc/doc.go.
# Do not edit this file manually.
# Run make docs-mesheryctl to regenerate.
...
func topLevelCommands(rootCmd *cobra.Command) map[string]*cobra.Command {
	...
	for _, cmd := range getVisibleCommands(rootCmd) {
...
func commandDoc(cmd *cobra.Command, alias string, children []*cobra.Command) yaml.MapSlice {
	...
	if len(children) > 0 {
		...
		for _, child := range children {
			subcommands = append(subcommands, yaml.MapItem{Key: commandKey(child), Value: commandDoc(child, "", getVisibleCommands(child))})
		}
...
func experimentalCommandDoc(cmd *cobra.Command) yaml.MapSlice {
	entry := yaml.MapSlice{}
	for _, child := range getVisibleCommands(cmd) {
		entry = append(entry, yaml.MapItem{Key: child.Name(), Value: commandDoc(child, "", getVisibleCommands(child))})
	}
	return entry
}
...
func commandTrackerDoc(rootCmd *cobra.Command) yaml.MapSlice {
	...
	if systemCmd, ok := topLevel["system"]; ok {
		...
		for _, name := range legacySystemAliasOrder() {
			if child, ok := children[name]; ok {
				tracker = append(tracker, yaml.MapItem{Key: "system-" + name, Value: commandDoc(child, "system "+name, getVisibleCommands(child))})
			}
		}
	}
	...
	for _, name := range legacyTopLevelOrder() {
		...
		tracker = append(tracker, yaml.MapItem{Key: commandKey(cmd), Value: commandDoc(cmd, "", getVisibleCommands(cmd))})
	}
...
func GenerateCommandTracker(rootCmd *cobra.Command, outputPath string) error {
	...
	content = append([]byte(commandTrackerHeader), content...)
