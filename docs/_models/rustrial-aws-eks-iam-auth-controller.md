---
title: Rustrial Aws Eks Iam Auth Controller
subtitle: Collaborative and visual infrastructure as design for Rustrial Aws Eks Iam Auth Controller
image: /extensions/models/rustrial-aws-eks-iam-auth-controller/icons/color/rustrial-aws-eks-iam-auth-controller-color.svg
aliases: /extensibility/integrations/rustrial-aws-eks-iam-auth-controller
docURL: https://docs.meshery.io/extensibility/integrations/rustrial-aws-eks-iam-auth-controller
description: 
integrations-category: Security & Compliance
integrations-subcategory: Security Identity Compliance
registrant: Artifact Hub
components: 
- name: iam-identity-mapping
  colorIcon: assets/img/integrations/rustrial-aws-eks-iam-auth-controller/components/iam-identity-mapping/icons/color/iam-identity-mapping-color.svg
  whiteIcon: assets/img/integrations/rustrial-aws-eks-iam-auth-controller/components/iam-identity-mapping/icons/white/iam-identity-mapping-white.svg
  description: 
components-count: 1
relationships: 
relationship-count: 0
featureList: [
  "Consolidates IAM identity mappings into aws-auth ConfigMap",
  "Enables GitOps management of IAM mappings",
  "Supports IAM roles and users"
]
howItWorks: "Integrates with IAM Auth Controller"
howItWorksDetails: "Streamlined IAM authentication and authorization management for EKS clusters"
categories: [integrations]
---
