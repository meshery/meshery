...
Six rules hold this together. Breaking any of them is a data-loss bug, not a style question.
...
- **The marker is Meshery's to write, never a client's.** Every read path hands its callers plaintext, so a secret reaching `EncryptCredentialSecret` already carrying `__mesheryEncryptedSecret` can only have come from a request body - the handler unmarshals one straight into `Credential.Secret`. It is rejected with `meshery-server-1486`, not treated as an already-sealed secret and not silently stripped: passing it through would store the rest of that secret in the clear and leave the row unreadable forever.
- **Plaintext rows keep reading, with no migration.** Every credential written before this shipped is plaintext and passes through the read path untouched; rows convert as they are rewritten. Converting a row does not remove the plaintext it replaced - SQLite does not zero a freed page, so on an upgraded install the earlier secret can stay recoverable from the raw file until a `VACUUM` reclaims it, which is exactly why a bulk migration would not fix it either (see [Credentials]({{< ref "concepts/logical/credentials.md#encryption-at-rest" >}})). Do not add one without a reason that survives the next point.
- **Every ciphertext names the key that sealed it.** `$TOKEN` is stable in practice but not guaranteed - a maintainer can rotate `GLOBAL_TOKEN`, and a locally built server links none and falls back to the `dev_token` default. A row sealed under one key and read under another is reported as `meshery-server-1484` naming the key identifier the ciphertext carries, not as corruption. The identifier *this* build derives stays out of that error - it is a function of the build-time token and the error reaches the API caller - and is logged server-side instead. `GetCredentialByID` fails on it, because its caller is about to authenticate with the value. Every other path degrades - the list, the delete, and the credential an update re-reads after its write has already committed - dropping that one secret, logging the reason and returning the rest, because one unreadable row must not make the credentials page permanently unreadable, and a durable write must not be reported as a failure.
