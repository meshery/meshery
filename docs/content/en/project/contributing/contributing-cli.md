...
The command pages and [reference index page](/reference/mesheryctl) are generated from Cobra command metadata. To add, remove, or update a command in the reference index, edit the command metadata in `mesheryctl/internal/cli/root/...`, then regenerate the CLI docs:

```bash
cd mesheryctl && make docs
```

Commit the generated command reference pages under `docs/content/en/reference/mesheryctl/` and the generated data file at `docs/data/mesheryctlcommands/cmds.yml`. The `cmds.yml` file is generated by `mesheryctl/doc/doc.go` and should not be manually edited unless you are changing the generator itself.
