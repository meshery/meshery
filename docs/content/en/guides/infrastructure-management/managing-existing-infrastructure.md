...
3. **MeshSync starts, in the deployment mode you chose.** Per context, you choose how MeshSync runs (recorded as the `meshsync_deployment_mode` metadata on the Connection):
   ...
   - **Operator**: Meshery Server installs the [Meshery Operator]({{< ref "concepts/architecture/operator/index.md" >}}) into the cluster (a Helm chart install into the `meshery` namespace, at a chart version the [repository actually publishes]({{< ref "guides/infrastructure-management/kubernetes-connection-lifecycle.md#which-operator-version-gets-installed" >}}) - never one that only matches the Server's release number). The operator in turn deploys [Meshery Broker]({{< ref "concepts/architecture/broker/index.md" >}}) (NATS) and MeshSync at versions it pins, and injects the Broker address into MeshSync as `BROKER_URL`. Administrators can also disable operator deployment server-wide with the `DISABLE_OPERATOR` environment variable on Meshery Server. In a restricted network, note that this step needs outbound access to the chart repository ([egress requirements]({{< ref "installation/production/networking-and-connectivity.md#egress-requirements" >}})).
