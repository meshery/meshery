---
title: Open Service Mesh
subtitle: Access all that Open Service Mesh has to offer
image: /extensions/models/open-service-mesh/icons/color/open-service-mesh-color.svg
docURL: https://docs.meshery.io/extensibility/adapters/osm
description: 
integrations-category: Cloud Native Network
integrations-subcategory: Service Mesh
registrant: Artifact Hub
components: 
- name: mesh-deployment
  colorIcon: /extensions/models/open-service-mesh/components/mesh-deployment/icons/color/mesh-deployment-color.svg
  whiteIcon: /extensions/models/open-service-mesh/components/mesh-deployment/icons/white/mesh-deployment-white.svg
  description: 
- name: mesh
  colorIcon: /extensions/models/open-service-mesh/components/mesh/icons/color/mesh-color.svg
  whiteIcon: /extensions/models/open-service-mesh/components/mesh/icons/white/mesh-white.svg
  description: 
- name: mesh-federation
  colorIcon: /extensions/models/open-service-mesh/components/mesh-federation/icons/color/mesh-federation-color.svg
  whiteIcon: /extensions/models/open-service-mesh/components/mesh-federation/icons/white/mesh-federation-white.svg
  description: 
featureList: [
  "Configuration an simplier understanding of your Open Service Mesh deployments and microservices",
  "Configure and chain Envoy WASM filters",
  "Conformance to Service Mesh Interface specifications"
]
howItWorks: "Collaborative Infrastructure as Code"
howItWorksDetails: "Collaboratively manage infrastructure with your coworkers synchronously sharing the same designs."
aliases:
- /extensibility/integrations/open-service-mesh
---
