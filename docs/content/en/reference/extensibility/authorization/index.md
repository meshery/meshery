...
When you are testing a new permission key locally, it helps to know where Meshery UI keeps that data in the browser. Keys are **not** stored in `localStorage`; the main client-side cache is **`sessionStorage`** while authentication is managed via **Cookies**.
