...
Self-generated API documentation for Meshery's GraphQL API. The API can be explored interactively using the [GraphQL Playground](http://localhost:9081/api/system/graphql/playground){:target="_blank"}. Documentation is generated from Meshery's GraphQL schema. Each table below documents a GraphQL type.
