...
**Relationships follow a schema-defined structure.** The [Relationship schema](https://github.com/meshery/schemas/tree/master/schemas/constructs/v1alpha3/relationship) specifies how relationships between components are expressed. Refer to the schema when defining new relationship types or selectors. See [Contributing to Schemas]({{site.baseurl}}/project/contributing/contributing-schemas) for details.
...
{% include alert.html type="warning" title="⚠️ Important: Avoiding Cyclical Hierarchical Relationships" content="<p><strong>Critical Requirement:</strong> Avoid creating cyclical dependencies in Hierarchical-Inventory relationships.</p><p>While components can have multiple parents in different relationship types, creating <strong>cyclical paths</strong> in Hierarchical-Inventory relationships causes system issues:</p><ul><li><strong>Infinite component instantiation loops</strong> - Components continuously spawn child instances</li><li><strong>Excessive resource creation</strong> - Hundreds or thousands of duplicate components are generated</li><li><strong>System instability</strong> - The design canvas becomes unusable due to performance degradation</li><li><strong>Poor user experience</strong> - Users cannot effectively manage or visualize their infrastructure</li></ul><p><strong>Problematic Pattern:</strong></p><p>Creating relationships where Component A patches Component B's configuration, and Component B also patches Component A's configuration (directly or through intermediate components) creates infinite loops.</p><p><strong>Example of Cyclical Issue ❌</strong></p><pre>Component A (Parent)<br>    ↓ patches configuration<br>Component B (Child)<br>    ↓ patches configuration (creates cycle)<br>Component A</pre><p><strong>Why does this happen?</strong> Each component's configuration patch invalidates the previous patch, causing the evaluation engine to continuously attempt reconciliation, resulting in an endless cycle of configuration updates and reevaluations.</p><p><strong>Best Practice:</strong> Design your Hierarchical-Inventory relationships as directed acyclic graphs (DAGs) where configuration flows in one direction without circular dependencies. If a component needs configuration from multiple sources, ensure the relationships don't create cycles.</p>" %}
