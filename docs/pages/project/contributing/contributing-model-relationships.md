...
{% include alert.html type="warning" title="⚠️ Important: Hierarchical-Inventory-Parent Constraint" content="<p><strong>Critical Requirement:</strong> Each component must have <strong>exactly ONE</strong> Hierarchical-Inventory-Parent relationship.</p><p>Adding multiple parent relationships to a single component creates <strong>cyclical dependencies</strong> that result in:</p><ul><li><strong>Infinite component instantiation loops</strong> - Components continuously spawn child instances</li><li><strong>Excessive resource creation</strong> - Hundreds or thousands of duplicate components are generated</li><li><strong>System instability</strong> - The design canvas becomes unusable due to performance degradation</li><li><strong>Poor user experience</strong> - Users cannot effectively manage or visualize their infrastructure</li></ul><p><strong>Example Scenario:</strong></p><p><strong>Correct Hierarchy ✅</strong></p><pre>VPC (Parent)<br>└── Subnet (Parent)<br>    └── NAT Gateway (Child) ← Has ONE parent</pre><p><strong>Incorrect Hierarchy ❌</strong></p><pre>VPC (Parent) ──┐<br>               ├── NAT Gateway (Child) ← Has TWO parents (creates cycle)<br>Subnet (Parent)─┘</pre><p>When a component has multiple Hierarchical-Inventory-Parents, the system attempts to apply configurations from both parents, which creates a recursive loop where:</p><ol><li>Parent A patches the child's configuration</li><li>Parent B patches the child's configuration</li><li>The patched configuration triggers reevaluation</li><li>Steps 1-3 repeat infinitely</li></ol><p><strong>Why does this happen?</strong> Each parent's configuration patch invalidates the previous patch, causing the evaluation engine to continuously attempt reconciliation, resulting in an endless cycle of configuration updates and reevaluations.</p><p><strong>Best Practice:</strong> Design your component hierarchies as strict trees with a single root and unique parent-child paths. If a component needs configuration from multiple sources, consider using <strong>Edge</strong> relationships or <strong>Sibling</strong> relationships instead.</p>" %}
