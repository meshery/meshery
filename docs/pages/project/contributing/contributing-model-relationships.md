...
{% include alert.html type="warning" title="⚠️ Important: Hierarchical-Inventory-Parent Constraint" content="<b>Critical Requirement:</b> Each component must have <b>exactly ONE</b> Hierarchical-Inventory-Parent relationship.<br/><br/>Adding multiple parent relationships to a single component creates <b>cyclical dependencies</b> that result in:<br/><ul><li><b>Infinite component instantiation loops</b> - Components continuously spawn child instances</li><li><b>Excessive resource creation</b> - Hundreds or thousands of duplicate components are generated</li><li><b>System instability</b> - The design canvas becomes unusable due to performance degradation</li><li><b>Poor user experience</b> - Users cannot effectively manage or visualize their infrastructure</li></ul><br/><b>Example Scenario:</b><br/><br/><b>Correct Hierarchy ✅</b><br/><pre>VPC (Parent)<br/>└── Subnet (Parent)<br/>    └── NAT Gateway (Child) ← Has ONE parent</pre><br/><b>Incorrect Hierarchy ❌</b><br/><pre>VPC (Parent) ──┐<br/>               ├── NAT Gateway (Child) ← Has TWO parents (creates cycle)<br/>Subnet (Parent)─┘</pre><br/>When a component has multiple Hierarchical-Inventory-Parents, the system attempts to apply configurations from both parents, which creates a recursive loop where:<br/><ol><li>Parent A patches the child's configuration</li><li>Parent B patches the child's configuration</li><li>The patched configuration triggers re-evaluation</li><li>Steps 1-3 repeat infinitely</li></ol><br/><b>Best Practice:</b> Design your component hierarchies as strict trees with a single root and unique parent-child paths. If a component needs configuration from multiple sources, consider using <b>Edge</b> relationships or <b>Sibling</b> relationships instead." %}
