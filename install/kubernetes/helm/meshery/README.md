...
| env.MESHSYNC_DEFAULT_DEPLOYMENT_MODE            | string | `"operator"`                                                                                                                                                                                        | Configure the deployment mode for Meshsync. Values: "embedded" (runs as a library inside Meshery Server process, one routine per connected k8s cluster); "operator" (deployed in managed k8s cluster, managed by Meshery Operator, one deployment per connected k8s cluster). See the [Meshsync deployment documentation](https://docs.meshery.io/concepts/architecture/meshsync#meshsync-deployment-mode) for more details. |
