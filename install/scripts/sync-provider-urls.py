...
def replace_allowed_return_to_hosts(text: str, active: "list[tuple[str, str]]", path: Path) -> str:
    """Regenerate the provider-ui ALLOWED_RETURN_TO_HOSTS array from the roster.

    These are the exact hostnames the post-auth /auth/redirect bounce may forward
    the browser (and the provider token) to; the page enforces an exact-match
    allowlist in provider-ui/lib/safe-redirect.mjs, mirroring meshery-cloud's
    credentialed-CORS trust model. Hostnames are derived from the active provider
    URLs, de-duplicated, in roster order.
    """
    hosts: list[str] = []
    for _, url in active:
        host = urlparse(url).hostname
        if not host:
            die(f"{path}: cannot derive a hostname from provider URL {url!r}")
    ...
    pat = re.compile(r'export const ALLOWED_RETURN_TO_HOSTS = \[[\s\S]*?\];')
    if not pat.search(text):
        die(f"{path}: ALLOWED_RETURN_TO_HOSTS array not found")
    return pat.sub(lambda _m: block, text, count=1)
...
def desired_contents() -> "dict[str, str]":
    ...
    plan = [
        ...
        ("provider-ui/lib/data-fetch.js",
         lambda t, p: replace_allowed_return_to_hosts(
             replace_js(t, "export const PROVIDER_URL = ", '"', primary, p), active, p)),
        ("ui/tests/e2e/env.js",
