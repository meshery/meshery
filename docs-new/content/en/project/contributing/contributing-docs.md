...
{{% alert color="warning" title="Don't Commit Gemfile" %}}
...
{{< code code=`{{ \"{% if product.title == 'Awesome Shoes' \" }}%}
...
{{< code code=`{{ \"{% for names in collection.names \" }}%}
  {{ \"{{ name.title \"}}}}
{{ \"{% endfor \" }}%}` >}}
