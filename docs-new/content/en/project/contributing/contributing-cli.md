...
{{< alert type="info" title="Meshery CLI Reference Documents" >}}<ul><li><a href="https://docs.google.com/spreadsheets/d/1q63sIGAuCnIeDs8PeM-0BAkNj8BBgPUXhLbe1Y-318o/edit#gid=0">Meshery Command Tracker</a>: Status of mesheryctl command implementation and platform compatibility.</li>
...
{{< code >}}var startCmd = &cobra.Command{
Use:   "start",
Short: "Start Meshery",
...
{{< alert type="warning" title="Avoid documentation being overwritten" >}}It is advised not to modify the changes in `docs` folder, rather should be done in `mesheryctl` folder as the changes will get overwritten by the CI workflows.{{< /alert >}}
...
{% raw %}{% include permalink-of-file %}{% endraw %}{{< /code >}}
...
{{< code >}}go test --short ./... -race -coverprofile=coverage.txt -covermode=atomic{{< /code >}}
