...
{{% alert color="warning" title="Avoid documentation being overwritten" %}}It is advised not to modify the changes in `docs` folder, rather should be done in `mesheryctl` folder as the changes will get overwritten by the CI workflows.{{% /alert %}}
...
{{< code code=`func TestPreflightCmdIntegration(t *testing.T) {
...
{{< code code=`go test --short ./... -race -coverprofile=coverage.txt -covermode=atomic` >}}
