...
const (
	...
	// may block the producer before the session is torn down.
...
// open session sockets, for deployments that serve the UI from a different
...
// WebSocket transport. A session socket hands its holder an interactive shell
...
// Socket is the server side of a session's WebSocket connection.
//
// It owns the underlying connection for the session's lifetime and enforces
...
// streaming call so a vanished client tears the remote session down promptly.
	...
	for {
		...
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseNormalClosure, websocket.CloseGoingAway, websocket.CloseAbnormalClosure) {
				s.log.Debug("session socket closed unexpectedly: " + err.Error())
			}
			return
		}
...
func (s *Socket) handleControl(payload []byte) {
	...
	if err := json.Unmarshal(payload, &msg); err != nil {
		s.log.Debug("discarding malformed session control frame: " + err.Error())
		return
	}
	...
	for {
		...
		case f, ok := <-s.outbound:
			...
			if err := s.ws.WriteMessage(f.messageType, f.payload); err != nil {
				s.log.Debug("session socket write failed: " + err.Error())
				s.shutdown()
				return
			}
...
// waits at most writeWait for room and then kills the session. Blocking
...
func (s *Socket) Close(code int, reason string) {
	...
	default:
		...
		case s.outbound <- frame{messageType: websocket.CloseMessage, payload: payload}:
			...
			// client learns *why* the session ended instead of seeing an
			// abnormal closure.
			select {
