...
func TestLimiterAllowsUpToMax(t *testing.T) {
	...
	if _, err := limiter.Acquire("alice"); err == nil {
		t.Fatal("third acquire succeeded, want a session-limit error")
	} else if code := errors.GetCode(err); code != ErrSessionLimitCode {
		t.Errorf("error code = %q, want %q", code, ErrSessionLimitCode)
	}
...
func TestLimiterRefusalReturnsUsableRelease(t *testing.T) {
	...
	if err == nil {
		t.Fatal("second acquire succeeded, want a session-limit error")
	}
...
func TestLimiterReleaseDropsKey(t *testing.T) {
	...
	if _, present := limiter.counts["alice"]; present {
		t.Error("counts still holds a zeroed entry for a user with no sessions")
	}
}
...
func TestLimiterIsConcurrencySafe(t *testing.T) {
	...
	if granted != max {
		t.Errorf("granted %d sessions concurrently, want exactly %d", granted, max)
	}
}
