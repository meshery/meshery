...
func TestCapabilitiesRunningPod(t *testing.T) {
	...
	caps, err := d.Capabilities(context.Background(), sessions.TargetRef{
...
func TestCapabilitiesNonPodResource(t *testing.T) {
	...
	if caps.Terminal || caps.Logs {
		t.Errorf("caps = %+v, want no supported session kinds", caps)
	}
	if caps.Reason == "" {
...
func TestCapabilitiesMissingPod(t *testing.T) {
	...
	_, err := d.Capabilities(context.Background(), sessions.TargetRef{
		Resource: "pod", Namespace: "default", Name: "ghost",
	})
	if err == nil {
		t.Fatal("Capabilities for a missing pod succeeded")
	}
	if code := errors.GetCode(err); code != sessions.ErrTargetNotFoundCode {
		t.Errorf("error code = %q, want %q", code, sessions.ErrTargetNotFoundCode)
	}
}
...
func TestCapabilitiesRequiresNamespaceAndName(t *testing.T) {
	...
	for _, target := range []sessions.TargetRef{
		...
		if code := errors.GetCode(err); code != sessions.ErrInvalidTargetCode {
			t.Errorf("error code = %q, want %q", code, sessions.ErrInvalidTargetCode)
		}
	}
}

func TestResolveContainer(t *testing.T) {
	caps := sessions.Capabilities{Containers: []string{"app", "sidecar"}, DefaultContainer: "app"}

	t.Run("defaults when unspecified", func(t *testing.T) {
		got, err := resolveContainer(sessions.TargetRef{}, caps)
		if err != nil || got != "app" {
	...
	t.Run("honours an explicit container", func(t *testing.T) {
		got, err := resolveContainer(sessions.TargetRef{Container: "sidecar"}, caps)
	...
	t.Run("rejects an unknown container", func(t *testing.T) {
		_, err := resolveContainer(sessions.TargetRef{Container: "nope"}, caps)
		if err == nil {
	...
	t.Run("rejects a pod with no containers", func(t *testing.T) {
		_, err := resolveContainer(sessions.TargetRef{}, sessions.Capabilities{})
		if err == nil {
...
func TestDriverRegistersItself(t *testing.T) {
	for _, kind := range sessions.Default.Kinds() {
		if kind == ConnectionKind {
			return
		}
	}
	t.Errorf("Default registry kinds = %v, want it to include %q", sessions.Default.Kinds(), ConnectionKind)
}
