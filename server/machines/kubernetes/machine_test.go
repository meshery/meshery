...
// GenerateKubeHandler errors Ã¢â€ â€™ GenerateK8sClientSet hits its log.Warn path Ã¢â€ â€™
...
// log through GenerateClientSetAction Ã¢â€ â€™ GenerateK8sClientSet. Any persisted
...
func TestAssignInitialCtx_AttachesLoggerBeforeClientSetAssignment(t *testing.T) {
	...
	// ProviderCtxKey: a typed-nil is fine Ã¢â‚¬â€ AssignControllerHandlers is only
	...
	// AssignClientSetToContext must fail for an unreachable/invalid context Ã¢â‚¬â€
	...
	// guard.
	if err == nil {
		t.Fatal("expected AssignInitialCtx to return an error for an unreachable K8s context, got nil Ã¢â‚¬â€ the regression guard is no longer exercising the panicking path")
	}
	if result != nil {
