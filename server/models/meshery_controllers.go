...
type MesheryControllersHelper struct {
	...
	// meshsyncInitMu serializes the initialization and teardown of ctxMeshsyncDataHandler
	// to prevent concurrent callers from creating multiple active MeshSync handlers.
	meshsyncInitMu sync.Mutex
...
func (mch *MesheryControllersHelper) AddMeshsyncDataHandlers(ctx context.Context, k8scontext K8sContext, userID, mesheryInstanceID core.Uuid, provider Provider) *MesheryControllersHelper {
	...
	if handlerIsNil {
		...
		mch.mu.RLock()
		alreadyInit := mch.ctxMeshsyncDataHandler != nil
		mch.mu.RUnlock()
		if !alreadyInit {
