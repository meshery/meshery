...
func (e *EventsPersister) GetAllEvents(eventsFilter *events.EventsFilter, userID uuid.UUID, sysID uuid.UUID) (*EventsResponse, error) {
	...
	countBySeverity, err := e.getCountBySeverity(userID, sysID, eventsFilter.Status)
	if err != nil {
		return nil, err
	}
...
func (e *EventsPersister) getCountBySeverity(userID, sysID uuid.UUID, eventStatus events.EventStatus) ([]*CountBySeverityLevel, error) {
	if eventStatus == "" {
