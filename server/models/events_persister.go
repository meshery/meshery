...
func (e *EventsPersister) GetEventTypes(userID uuid.UUID, ownerUserID uuid.UUID) (map[string]interface{}, error) {
	eventTypes := make(map[string]interface{}, 2)
	var categories, actions []string
	err := e.DB.Table("events").Distinct("category").Where("user_id = ? ", userID, ownerUserID).Find(&categories).Error
	if err != nil {
		return nil, err
	}
...
func (e *EventsPersister) GetAllEvents(eventsFilter *events.EventsFilter, userID uuid.UUID, ownerUserID uuid.UUID) (*EventsResponse, error) {
	eventsDB := []*events.Event{}
	finder := e.DB.Model(&events.Event{}).Where("user_id = ? OR user_id = ?", userID, ownerUserID)
