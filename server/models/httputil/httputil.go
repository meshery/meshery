...
//
// Never use http.Error from handlers or the provider layer â€” it emits
...
// response parsers â€” notably RTK Query's default baseQuery, which parses by
// Content-Type and treats application/json as JSON â€” from choking on error
...
// produce a JSON body â€” they just carry only the .Error() string, matching
...
func WriteMeshkitError(w http.ResponseWriter, err error, status int) {
	...
	if code := meshkiterrors.GetCode(err); code != "" && code != "None" {
		...
		if short := meshkiterrors.GetSDescription(err); short != "" && short != "None" {
			// Use ShortDescription as the user-facing `error` when available â€”
...
// "Database reset successful") â€” promote them to a structured message.
