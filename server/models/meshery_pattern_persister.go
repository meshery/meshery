...
func (mpp *MesheryPatternPersister) GetMesheryPatterns(search, order string, page, pageSize uint64, updated_after string) ([]byte, error) {
	...
	query := mpp.DB.Where("visibility = 'private'").Where("updated_at > ?", updated_after).Order(order)
