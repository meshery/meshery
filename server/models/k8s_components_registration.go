...
func (cg *ComponentsRegistrationHelper) RegisterComponents(ctxs []*K8sContext, regFunc k8sRegistrationFunction, eb *events.EventStreamer) {
	for _, ctx := range ctxs {
		...
		cg.log.Info("registering for k8s native components started for contextID: ", ctxID)
		...
		go func(ctx *K8sContext) {
			...
			defer func() {
				...
				cg.log.Info("registration for k8s native components completed for contextId: ", ctxID)
				req := meshes.EventsResponse{
					Component:            "core",
					ComponentName:        "Kubernetes",
					EventType:            meshes.EventType_INFO,
