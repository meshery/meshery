...
func LocalPathWalker(catalogDir string, log logger.Handler, file string) ([][]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to walk through catalog directory: %w", err)
	}
	return contents, nil
}

// SeedContent - to seed various contents with local provider-like patterns, filters
func (l *DefaultLocalProvider) SeedContent(log logger.Handler) {
	...
	catalogDir := filepath.Join(currentFilePath, "..", "..", "docs", "catalog")

	for _, seedContent := range seedContents {
		go func(comp string, log logger.Handler, seededUUIDs *[]uuid.UUID) {
			switch comp {
			case "Pattern":
				contents, err := LocalPathWalker(catalogDir, log, "design.yml")
				if err != nil {
				...
				for i, content := range contents {
					wg.Add(1)
					go func(content []byte, index int) {
						defer wg.Done()
						id, _ := uuid.NewV4()
