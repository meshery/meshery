...
func NewPatternFileFromK8sManifest(regManager *meshmodel.RegistryManager, data string, ignoreErrors bool) (Pattern, error) {
	pattern := Pattern{
		Name:     "Autogenerated",
	...
	for _, manifestYAML := range manifests {
		...
		name, svc, err := createPatternServiceFromK8s(regManager, manifest)
		if err != nil {
			if ignoreErrors {
				continue
			}
...
func createPatternServiceFromK8s(regManager *meshmodel.RegistryManager, manifest map[string]interface{}) (string, Service, error) {
