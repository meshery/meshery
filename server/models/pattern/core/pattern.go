...
func NewPatternFileFromK8sManifest(data string, fileName string, ignoreErrors bool, reg *registry.RegistryManager) (pattern.PatternFile, error) {
	...
	if len(pattern.Components) == 0 {
		if resourceCount > 0 {
			return pattern, ErrCreatePatternService(fmt.Errorf("none of the %d resources in the manifest could be resolved to known components. Ensure a Kubernetes cluster is connected so component definitions are available in the registry", resourceCount))
		}
