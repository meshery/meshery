...
func NewPatternFileFromK8sManifest(data string, registryManager *meshmodel.RegistryManager, ignoreErrors bool) (Pattern, error) {
	pattern := Pattern{
		Name:     "Autogenerated",
	...
	for _, manifestYAML := range manifests {
		...
		name, svc, err := createPatternServiceFromK8s(manifest, registryManager)
		if err != nil {
			if ignoreErrors {
				continue
			}
...
func createPatternServiceFromK8s(manifest map[string]interface{}, rm *meshmodel.RegistryManager) (string, Service, error) {
	...
	entities := rm.GetEntities(&meshmodelv1alpha1.ComponentFilter{
	...
	var comp meshmodelv1alpha1.ComponentDefinition
	for _, ent := range entities {
		c, ok := ent.(meshmodelv1alpha1.ComponentDefinition)
		if ok {
			comp = c
			if c.Model.Name != "kubernetes" { //If the apiVersion and kind pair existed with a model other than "kubernetes" then give the model a priority
				break
			}
		}
	}
