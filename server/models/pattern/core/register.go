...
func RegisterMesheryOAMTraits() error {
	...
	// accordingly
	rootPath, _ := filepath.Abs("../oam/core/traits")
...
func RegisterMesheryOAMWorkloads() error {
	...
	// if the file is moved then this path MUST be changed
	// accordingly
	rootPath, _ := filepath.Abs("../oam/core/workloads")

	return registerMesheryServerOAM(rootPath, []string{"application"}, RegisterWorkload)
}
func RegisterK8sOAMWorkloads() error {
	rootPath, _ := filepath.Abs("../oam/kubernetes/workloads")

	return registerK8sOAM(rootPath, RegisterWorkload)
}
func registerK8sOAM(rootpath string, regFn func([]byte) error) error {
	...
	for _, item := range items {
		if !item.IsDir() {
			path := filepath.Join(rootpath, item.Name())
			...
			if strings.HasSuffix(path, "_definitions.k8s.json") {
				...
				path = strings.TrimSuffix(path, "_definitions.k8s.json") + "_schema.k8s.json"
				...
				data := map[string]interface{}{
					...
					"metadata": map[string]string{
						"adapter.meshery.io/name": "kubernetes",
					},
				}
