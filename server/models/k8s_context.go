...
func FlushMeshSyncData(ctx context.Context, k8sContext K8sContext, provider Provider, eventsChan *Broadcast, userID string, mesheryInstanceID *uuid.UUID, log logger.Handler) {
	...
	if mesheryInstanceID != nil {
		instanceID = *mesheryInstanceID
	}
	...
	if !ok || len(k8sctxs) == 0 {
		event := events.NewEvent().ActedUpon(ctxUUID).FromSystem(instanceID).WithSeverity(events.Error).WithCategory("meshsync").WithAction("flush").WithDescription("No Kubernetes context specified, please choose a context from context switcher").FromUser(userUUID).Build()
	...
	if refCount == 1 {
		if provider.GetGenericPersister() == nil {
			...
			event := events.NewEvent().ActedUpon(ctxUUID).FromSystem(instanceID).WithSeverity(events.Error).WithCategory("meshsync").WithAction("flush").WithDescription(fmt.Sprintf("Error flushing MeshSync data for %s", ctxName)).FromUser(userUUID).WithMetadata(map[string]interface{}{
		...
		event := events.NewEvent().ActedUpon(ctxUUID).FromSystem(instanceID).WithSeverity(events.Informational).WithCategory("meshsync").WithAction("flush").WithDescription(fmt.Sprintf("MeshSync data flushed for context %s", ctxName)).FromUser(userUUID).Build()
