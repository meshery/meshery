...
func GetModelDirectoryPaths(modelPath string) ([]string, error) {
	...
	for _, modelDir := range modelsDirs {
		...
		for _, versionDirPath := range sortedVersionDirs {
			...
			componentsDirPath := filepath.Join(modelDefDirPath, "components")
			if meshkitUtils.IsDirectoryNonEmpty(componentsDirPath) {
				latestWithComponentsDirPath = modelDefDirPath
				break
			}
		}
