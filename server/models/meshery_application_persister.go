...
func (maap *MesheryApplicationPersister) GetMesheryApplications(search, order string, page, pageSize uint64, updated_after string) ([]byte, error) {
	...
	query := maap.DB.Where("updated_at > ?", updated_after).Order(order)
