...
func TestSocketRoundTrip(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		defer socket.Close(CloseConsoleEnded, "done")
	...
	// 5. The console ends with an exit frame, then a close frame carrying the
	//    console-ended code.
	messageType, payload, err = client.ReadMessage()
	if err != nil {
	...
	} else if !websocket.IsCloseError(err, CloseConsoleEnded) {
		t.Fatalf("close error = %v, want close code %d", err, CloseConsoleEnded)
	}
...
// console using the victim's ambient cookie.
...
func TestUpgradeAcceptsSameOrigin(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		socket.Start(r.Context())
		socket.Close(CloseConsoleEnded, "done")
	}))
	defer server.Close()
