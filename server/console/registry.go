...
type ConnectionContext struct {
	...
	// credential from the provider. Consoles therefore run with the requesting
...
func (r *Registry) Driver(ctx context.Context, cc ConnectionContext) (Driver, error) {
	if cc.Connection == nil {
		return nil, ErrInvalidTarget("no connection was resolved for this console request")
	}
...
// wherever consoles are served — the same shape as database/sql drivers and
