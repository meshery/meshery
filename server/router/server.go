...
func NewRouter(_ context.Context, h models.HandlerInterface, port int, g http.Handler, gp http.Handler) *Router {
	...
	gMux.Handle("/api/meshmodel/update/status", h.ProviderMiddleware((http.HandlerFunc(h.UpdateIgnoreStatus)))).Methods("POST")                        //For backwards compatibility with previous registrants
