...
func NewRouter(_ context.Context, h models.HandlerInterface, port int, g http.Handler, gp http.Handler) *Router {
	...
	meshmodelsFS := http.FileServer(http.Dir("../../ui/public/static/img/meshmodels"))
	gMux.PathPrefix("/static/img/meshmodels").Handler(http.StripPrefix("/static/img/meshmodels", meshmodelsFS)).Methods("GET")
	gMux.PathPrefix("/").
