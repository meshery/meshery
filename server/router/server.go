...
func NewRouter(_ context.Context, h models.HandlerInterface, port int, g http.Handler, gp http.Handler) *Router {
	...
	gMux.Handle("/api/integrations/connections", h.ProviderMiddleware(h.AuthMiddleware(h.SessionInjectorMiddleware(h.BodySizeLimiterMiddleware(h.SaveConnection)), models.ProviderAuth))).
		Methods("POST")
