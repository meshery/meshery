...
func ErrPatternDeploy(err error, patternName string) error {
	return errors.New(ErrPatternDeployCode, errors.Alert,
		[]string{fmt.Sprintf("Unable to deploy the selected design \"%s\"", patternName)},
		[]string{err.Error()},
		[]string{"Connection Error: There was an error connecting to the selected target platform (i.e. Kubernetes cluster(s)).",
			"This connection might not be assigned to the selected environment."},
		[]string{"Verify that the Kubernetes connection status is 'Connected' or try uploading a new kubeconfig.",
			"Assign the current Kubernetes connection to the selected environment."})
}
