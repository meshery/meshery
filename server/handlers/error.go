...
func ErrInvalidFilePath() error {
	return errors.New(ErrInvalidFilePathCode, errors.Alert, []string{"Invalid or forbidden file path"}, []string{"The requested file path resolves outside the directory Meshery is permitted to serve files from"}, []string{"The file query parameter attempted to traverse outside ~/.meshery/logs, resolved through a symlink that escapes it, targeted a sibling directory, or did not reference an existing Meshery log file"}, []string{"Request only files generated by Meshery under ~/.meshery/logs, for example registration or model-generation logs"})
}
