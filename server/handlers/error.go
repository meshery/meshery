...
const (
	...
	ErrInvalidConnectionKindCode           = "meshery-server-1430"
)
...
func ErrInvalidConnectionKind(err error) error {
	return errors.New(ErrInvalidConnectionKindCode, errors.Alert, []string{"Connection kind is not supported for this operation"}, []string{err.Error()}, []string{"The connection kind must be \"kubernetes\" to perform meshsync deployment mode changes"}, []string{"Verify that the connection kind is \"kubernetes\" before attempting meshsync deployment mode changes"})
}
