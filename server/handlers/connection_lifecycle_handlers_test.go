...
func TestDeleteContext_SuccessRemovesTracker(t *testing.T) {
	...
	reqCtx = context.WithValue(reqCtx, trackerCleanupDoneKey, trackerCleanupDone)
	req = req.WithContext(reqCtx)
	rec := httptest.NewRecorder()
	...
	// Wait for the tracker-removal goroutine, without a timing assumption.
	select {
	case <-trackerCleanupDone:
	case <-time.After(5 * time.Second):
		t.Fatal("tracker cleanup did not finish")
	}
