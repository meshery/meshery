...
func (h *Handler) DeleteContext(w http.ResponseWriter, req *http.Request, _ *models.Preference, user *models.User, provider models.Provider) {
	...
	deleteEvent, deleteGenerationCtx, sendErr := inst.SendEventWithGeneration(detachedCtx, machines.Delete, done)
	if sendErr != nil {
	...
	// Retain the StateMachine in the tracker while the background Delete cleanup
	// runs, and wait for the 'done' channel the Delete transition's cleanup
	// goroutine closes. A reconnect accepted during cleanup adopts this same
	// StateMachine: it becomes the lifecycle owner, and its own side effects
	// queue behind the cleanup on ActionMutex.
	go func() {
		<-done

		// Cleanup is finished. Remove the StateMachine only if this Delete is
		// still the machine's generation. deleteGenerationCtx came back from
		// SendEventWithGeneration under the machine lock, so it is this Delete's
		// own generation; a reconnect that has adopted the machine rotates the
		// generation and the comparison below then refuses the removal.
