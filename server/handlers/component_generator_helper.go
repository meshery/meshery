...
func setDefaultValues(model *meshkitRegistryUtils.ModelCSV) {
	...
	var capabilityDefault []interface{}
	if err := json.Unmarshal(schema, &schemaMap); err == nil {
		if def, ok := schemaMap["default"].([]interface{}); ok {
			capabilityDefault = def
		}
	}

	if model.Capabilities == "" {
		if capabilityDefaultJSON, err := json.Marshal(capabilityDefault); err == nil {
			model.Capabilities = string(capabilityDefaultJSON)
		}
	}
