...
func (h *Handler) NotifySmOfConnectionStatusChange(ctx context.Context, userID core.Uuid, provider models.Provider, token string, connection *connections.ConnectionPayload) (events.Event, error) {
	...
	if connection.Status != "" {
		...
		// whichever way its machine failed. A machine whose Context was never
		// assigned is still tracked, and nothing can drive it afterwards: the
		// generation-guarded removal below and the one in DeleteContext both run
		// only after a SendEvent that cannot succeed without a Context. Drop the
		// entry here instead.
		...
		event, transitionGenerationCtx, err := inst.SendEventWithGeneration(detachedCtx, machines.EventType(helpers.StatusToEvent(connection.Status)), done)
		if err != nil {
			h.log.Error(err)
			if event != nil {
				return *event, err
		...
		if connection.Status != connections.DELETED {
			// A delete cleanup running concurrently may have removed this entry
			// between the tracker handing us this machine and the transition
			// above committing. The machine is ours now, so re-assert ownership
			// rather than leave a live connection untracked and let the next
			// request build a second machine for it.
			smInstanceTracker.AddIfAbsent(inst.ID, inst)
		}

		if connection.Status == connections.DELETED {
			// Retain the StateMachine in the tracker while the background Delete cleanup runs.
			deleteGenerationCtx := transitionGenerationCtx
			go func() {
				<-done
