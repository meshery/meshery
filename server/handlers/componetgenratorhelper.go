...
func writeMessageString(response *models.RegistryAPIResponse) string {
	...
	if response.EntityCount.ModelCount > 0 {
		if response.EntityCount.CompCount == 0 && response.EntityCount.RelCount == 0 && response.EntityCount.TotalErrCount == 0 {
			return "Model won't be registered because there was no Component or Relationship."
		}
	...
	if response.EntityCount.CompCount > 0 || response.EntityCount.RelCount > 0 {
		...
		if response.EntityCount.RelCount > 0 {
			if response.EntityCount.CompCount > 0 {
...
func ErrMsgContruct(response *models.RegistryAPIResponse) string {
	...
	errors := []string{}
	if response.EntityCount.ErrModelCount > 0 {
	...
	if response.EntityCount.ErrCompCount > 0 {
	...
	if response.EntityCount.ErrRelCount > 0 {
	...
	if unknownErrors := response.EntityCount.TotalErrCount - (response.EntityCount.ErrCompCount + response.EntityCount.ErrRelCount + response.EntityCount.ErrModelCount); unknownErrors > 0 {
